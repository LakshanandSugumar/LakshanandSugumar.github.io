...
- **PIC18F57Q83 (U22, center)** — QFN-48 package placed at the center of the board. All peripheral signal traces fan out from the QFN pad ring. Power and ground pins connect directly to short traces leading to bypass capacitors placed immediately adjacent to the IC.
...
- **Switches (SW11–SW14, top left)** — Tactile switches grouped along the left edge for easy operator access. Each switch has a dedicated pull-up resistor placed immediately adjacent.
...
- **I²C pull-up resistors (RF1, R11)** — Placed close to the SDA and SCL lines to minimize stub length on the shared software I²C bus.
...
The soldermask top layer exposes all SMD pads and connector through-holes. The PIC18F57Q83 QFN pad pattern is clearly visible at center. All component pads are correctly opened with no soldermask slivers between fine-pitch QFN pads.
...
### Figure 06: Bare PCB (Post-Fabrication)

![Empty PCB](emptypcb.jpeg)

The fabricated bare board prior to component assembly. The two-layer stackup, soldermask, and silkscreen are visible. The QFN-48 pad array at center and the regulator footprint at lower-left match the design files above.

---

### Figure 07: Assembled PCB (Populated and Wired)

![Wired PCB](wiredpcb.jpeg)

The fully populated and wired board as demonstrated at the Innovation Showcase. All SMD components are soldered to the top layer. The BNO055 IMU, SH1106 OLED, and HDC2080 are connected via the I²C bus headers. The ribbon cable connectors route UART TX/RX and 3.3 V power to the upstream and downstream boards in the daisy chain.

---

## Design Decisions

**Component placement.** All active ICs are placed on the top layer to allow single-sided reflow assembly. The PIC18F57Q83 is centered on the board so that signal traces to all peripherals are approximately equal length and no one group of peripherals requires long routing detours.
...
**Bypass capacitor placement.** 0.1 µF ceramic bypass capacitors are placed immediately adjacent to each VDD pin on the PIC18F57Q83, BNO055, SH1106, and HDC2080. The 22 µF bulk capacitor is placed at the regulator output.

**I²C bus routing.** SDA and SCL traces are kept short and routed together as a pair from the PIC to the sensor and display connectors. Pull-up resistors are placed close to the MCU end of the bus to minimize the stub that the pull-up sees. All three I²C devices — BNO055, SH1106, and HDC2080 — share this single bus with unique addresses (0x28, 0x3C, 0x40).
...
**Hardware I²C fallback.** The hardware MSSP I²C peripheral on the PIC18F57Q83 was non-functional post-fabrication due to a clock source configuration error in MCC Melody. All I²C communication was handled via a software bit-bang implementation on RC3 and RC4 with no change to the PCB. The physical routing remained valid for both hardware and software I²C use.
